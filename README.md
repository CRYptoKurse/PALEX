...
│   ├── ir/                     # Intermediate representation (Sprint 4)
│   └── main.py                 # Entry point
├── tests/
│   ├── lexer/                  # Lexer tests
...
python src/main.py -i examples/program.src --ir --ir-format text

# Generate IR Graphviz CFG
python src/main.py -i examples/program.src --ir --ir-format dot -o program.cfg.dot

# Output IR as JSON
python src/main.py -i examples/program.src --ir --ir-format json -o program.ir.json
...
| `--ir` | Generate intermediate representation from the parsed AST |
| `--ir-format [text\|dot\|json]` | IR output format (default: text) |
| `--optimize` | Run IR optimization phase on the generated IR |
| `--ir-stats` | Show IR statistics after generation |
...
The IR uses a three-address code form with the following instruction classes:

- Arithmetic: `ADD`, `SUB`, `MUL`, `DIV`, `MOD`
- Logical: `AND`, `OR`, `NOT`
- Comparison: `CMP_EQ`, `CMP_NE`, `CMP_LT`, `CMP_LE`, `CMP_GT`, `CMP_GE`
- Memory: `LOAD dest, [addr]`, `STORE [addr], src`, `ALLOCA dest, size`
- Control Flow: `JUMP label`, `JUMP_IF cond, label`, `JUMP_IF_NOT cond, label`, `PHI dest, (val1, label1), (val2, label2), ...`
- Function: `CALL dest, func, args...`, `RETURN value`, `PARAM index, value`
- Data Movement: `MOVE dest, src`

The IR also preserves parameter types in function headers and includes human-readable comments for source-level correspondence.
