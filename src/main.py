...
def main():
    parser = argparse.ArgumentParser(description="MiniCompiler - Sprint 2")
    ...
    if parser_obj.errors and args.verbose:
        print(f"Parsing completed with {len(parser_obj.errors)} error(s).", file=sys.stderr)
        sys.exit(1)
    elif parser_obj.errors:
        sys.exit(1)
    else:
        sys.exit(0)
